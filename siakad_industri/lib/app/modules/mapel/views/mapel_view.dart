...
import 'package:siakad_industri/app/data/repositories/repository_karyawan.dart';
...
class MapelView extends GetView<MapelController> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        //ini buat manggil controller nya dari mapel crotroler
        padding: const EdgeInsets.all(8.0),
        child: Obx(
          //biar kalau di klik itu bisa ganti // value yang mau di listen  sama app nnya
          () {
            ...
            if (controller.mapel.value.data == null ||
              ...
              //kalau controller.mapelnya eror pakai .value
...
void _showUpdateModal(BuildContext context, DataMapel element) {
  ...
  final _guruIdController = TextEditingController(
      text: element.guruId != null && element.guruId!.isNotEmpty
          ? element.guruId![0].toString()
          : '');
        ...
        actions: [
          ElevatedButton(
            onPressed: () async {
              ...
              bool isSuccess =
                  await repository.updateKaryawan(element.id!, updatedData);
              if (isSuccess) {
